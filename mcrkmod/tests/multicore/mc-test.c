...
int main(int argc, char *argv[]){
	...
	for (cpuid = 0; cpuid < NUM_PROCESS; cpuid++) {
		...
		C = (2 + cpuid) * MILLISEC_TO_NANOSEC;
		//C = 5 * MILLISEC_TO_NANOSEC;
		T = (100 + cpuid * 3) * MILLISEC_TO_NANOSEC;
		D = (100 + cpuid * 3) * MILLISEC_TO_NANOSEC;
	...
	/*
	for (cpuid = 0; cpuid < NUM_PROCESS; cpuid++) {
	//for (cpuid = NUM_PROCESS - 1; cpuid >= 0; cpuid--) {
		//printf("pid:%d\n", pid[cpuid]);
		kill(pid[cpuid], 9);
	}*/
