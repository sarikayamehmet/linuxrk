		...
		for(i=1; i<argc; i++) {
			...
			if(strlen(argv[i])>=2) {
				if(argv[i][1] == '?'
				   || argv[i][1] == 'h' 
				   || argv[i][1] == 'H') {
					printf("<usage>: create-cpu-reservation -C=<compute_time> -T=<period> -D=<deadline> -R=<resource_set_name>\n");
